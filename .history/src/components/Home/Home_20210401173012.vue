<template>
  ...
  <el-checkbox v-model="checked" class="agreePro">
     同意
    <a href="https://st.music.163.com/official-terms/service">《服务政策》</a>
  ...
  <el-form-item label="昵称" prop='nickname'>
    <el-input v-model="signUpForm.nickname" class="nicknameInput"></el-input>
  </el-form-item>
   <el-form-item label="密码" prop=password>
    <el-input class="passwordInput" type="password" v-model="signUpForm.password"></el-input>
  </el-form-item>
  <el-form-item label="电话" prop='phone'>
  ...
  <el-form-item class="codeItem" label="验证码" prop='code'>
    <el-input class="codeInput" v-model="signUpForm.code">  
    </el-input>
    <el-button type="primary" @click="faYanZheng">发送验证码</el-button>
  </el-form-item>
  
  </el-form-item>
    <el-button type="primary" class="zhuce" @click="zhuceMusicId">注册</el-button>
  </el-form>
</el-dialog>
...
export default {
  setup(props,context){
     ...
     const faYanZheng =async () => {
       const {data:res} = await axios.get('/captcha/sent?phone='+signUpForm.phone);
       if(res.code ==200){
     ...
     const zhuceMusicId =async ()=> {
        const {data:yanzhengYanzhengma} = await axios.get('/captcha/verify?phone='+signUpForm.phone+'&captcha='+signUpForm.code);
        const {data:res} = await axios.post('/register/cellphone',signUpForm); 
        console.log(res)
     }
     // 登录表单提交信息
     ...
     const submitLoginForm = async () => {
        ...
        onMounted
        console.log(myRef);
        context.$refs.loginFormRef.resetFields();
     }   
     return {
       //登录
       loginDialogVisible,
