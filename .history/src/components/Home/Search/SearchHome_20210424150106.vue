        ...
        >
         单曲
          <div v-if="activeName==1">
              <ul class="singleUl">
                <li v-for="(item,index) in searchMusicList.songs" :key="index" @click="handleSingleMusicList($event,index)">
                  {{
                  item.name
                  }}
                  </li>
...
export default {
  ...
  methods: {
    ...
    // 点击单曲列表播放音乐
  async  handleSingleMusicList(e,index){
      const {data:res} = await this.$axios.get('http://localhost:3000/song/url?id='+this.searchMusicList.result.songs[index].id);
      const {data:lrc} = await this.$axios.get('http://localhost:3000/lyric?id='+this.searchMusicList.result.songs[index].id);
      this.musicObjParams.lrc = lrc.lrc.lyric;
      this.musicObjParams.title = this.searchMusicList.result.songs[index].name;
