<template>
  <div>
    ...
    >
      <el-table-column prop="songName" label="音乐标题" sortable width="300">
      </el-table-column>
      <el-table-column prop="artist" label="歌手" sortable width="300">
      </el-table-column>
      <el-table-column prop="album" label="专辑" sortable> </el-table-column>
      <el-table-column prop="fileSize" label="大小"></el-table-column>
      <el-table-column prop="addTime" label=""></el-table-column>
    </el-table>
  </div>
</template>
...
import { callWithAsyncErrorHandling, onMounted, reactive, ref } from "vue";
export default {
  setup() {
    ...
    const getCloudPan = async () => {
      ...
      parseOne.value =
        index > 0 ? parseOne.value.slice(0, index + 2) : parseOne.value;
      maxSize.value = parseInt(
        yunpan.value.maxSize / (1024 * 1024 * 10 * 10 * 10)
      );
      data.value = res.data;
      data.value.forEach((element) => {
        element.fileSize = element.fileSize / (1024 * 1024);
        element.fileSize= element.fileSize.toString();
        const index = element.fileSize.indexOf(".");
        element.fileSize =
          index > 0 ? element.fileSize.slice(0, index + 2) : element.fileSize;
        element.fileSize = element.fileSize+'MB';
        // console.log(element.fileSize);
        const time = new Date(element.addTime);
        element.addTime = `${time.getFullYear()}年${time.getMonth()+1}月${time.getDate()}日 `;
