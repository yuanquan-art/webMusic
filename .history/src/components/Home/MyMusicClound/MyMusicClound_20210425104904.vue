<template>
  <div>
    ...
    <div class="block">
      ...
      <span>{{(yunpan.value.size/(1024*1024*10*10))}}G/{{parseInt(yunpan.value.maxSize/(1024*1024*10*10*10))}}G</span>
    </div>
...
export default {
  setup() {
    ...
    const getCloudPan = async () => {
      ...
      //   ((res.size/1024) / (res.maxSize / 1024)) * 10000
      // );
     parseOne.value = yunpan.value.size/(1024*1024*10*10);
     console.log(parseOne.value)
    };
    getCloudPan();
