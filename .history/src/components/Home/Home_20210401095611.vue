<template>
  ...
  width="25%"
  :before-close="loginDialogClosed">
  <!-- 对话主体 -->
...
<!-- 点击免费注册弹出注册对话框 -->
  ...
  width="25%"
  :before-close="signUpDialogClosed">
  <!-- 对话主体 -->
  <p3 class="zhuce">注册</p3>
  <!-- 登录表单 -->
  ...
  setup(props,context){
     ...
     const showSignUpDialogVisible =async () => {
        if(checked.value !== true){
          ElMessage.success('请同意用户协议');
          return;
        }
        loginDialogVisible.value = false;
        //获取国家编码
        const {data: res} = await axios.get("/countries/code/list");
        //所有国家列表追加在一起
        // const countries = reactive([]);
        console.log(res)
        console.log(res.data[0].countryList[0])
        const countries = [];
        for(let i=0;i<res.data[i].length;i++){
          for(let j=0;j<res.data[i].countryList[j].length;j++){
            // countries.push(res.data[i].countryList[j]);
            
            console.log(res.data[i].countryList[j])
          }
        }
