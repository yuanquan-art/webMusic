        ...
        <el-container>
          ...
          <el-main class="biaotiMessage">
            <p>
              ...
              <span class="musicname">我喜欢的音乐</span>
            </p>
            <p>
              <img
                :src="avatarUrl"
...
export default {
  ...
  methods: {
    ...
    async getMusicListXQ() {
      const { data: resA } = await this.$axios.get(
        "http://localhost:3000/likelist?uid=" +
          this.$route.query.id
      );
      // resA.ids.forEach(element => {
      //     console.log(element)
      // });
        ...
        this.nickname = this.$store.state.loginForm.nickname;
        this.avatarUrl = this.$store.state.loginForm.avatarUrl;
