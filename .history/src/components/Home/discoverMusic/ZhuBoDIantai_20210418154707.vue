       ...
       <!-- 轮播图 -->
        ...
        <div class="tjmusic">
          ...
          <div
            ...
            :key="item.id"
            @click="moneyJingchai(item.id,item.picUrl)"
          >
...
export default {
  ...
  methods: {
    ...
    async getDiantaiLunbotuList(){
        const {data:res} = await this.$axios.get('http://localhost:3000/dj/banner');
        this.diantaBannerList = res.data;
    },
    // 获取电台付费精品4图展示
    async getDiantaiMoneySpecilFourPic(){
      const {data:res} = await this.$axios.get('http://localhost:3000/dj/paygift?limit=4');
    ...
    async getChuangzuoFanchangSixPicList(){
      const {data:res} = await this.$axios.get('http://localhost:3000/dj/radio/hot?cateId=2001');
      for(let i=0;i<6;i++){
    ...
    async getDiantaiMoneySpecilSixPic(){
      const {data:res} = await this.$axios.get('http://localhost:3000/dj/paygift?limit=10');
      for(let i=4;i<10;i++){
