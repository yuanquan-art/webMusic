...
export default {
  ...
  methods: {
    ...
    async getChuangzuoFanchangSixPicList(){
      const {data:res} = await this.$axios.get('/dj/radio/hot?cateId=2001');
      for(let i=0;i<6;i++){
    ...
    async getDiantaiMoneySpecilSixPic(){
      const {data:res} = await this.$axios.get('/dj/paygift?limit=10');
      for(let i=4;i<10;i++){
        this.DiantaiMoneySpecilSixPicList.push(res.data.list[i]);
      }
      console.log(this.DiantaiMoneySpecilSixPicList)
    },
  }
}
</script>
