      ...
      <div class="tab">
        ...
        <div class="xinMusic" :class="currentIndex == 0 ? 'currentdiv' : ''">
          <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane :label="item.label" :name="item.name" v-for="(item,index) in areaList" :key="index">
                <ul>
                    <li class="areaLi" v-for="(item,index) in newMusicFast" :key="index">
        ...
        <div class="xinMusic" :class="currentIndex == 1 ? 'currentdiv' : ''">
            ...
            <el-tab-pane :label="item.label" :name="item.name" v-for="(item,index) in areaDieUp" :key="index">
                <ul>
                    <li class="areaLi" v-for="(item,index) in newMusicDieupList" :key="index">
...
export default {
             ...
             methods: {
               ...
               async getNewMusicFast() {
                   const {data: res } = await this.$axios.get('http://localhost:3000/top/song?type='+this.activeName);
                   this.newMusicFast = res.data;
               },
               // 获取新碟上架列表
               async getNewDieUp(){
                  const {data:res} = await this.$axios.get('http://localhost:3000/album/new?&limit=500');
