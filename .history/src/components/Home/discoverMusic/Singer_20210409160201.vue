...
export default {
    ...
    methods: {
        // 获取歌手列表
      async getSingeList(){
         const {data:res} = await this.$axios.get('/artist/list?limit=100&offset=1');
      ...
      async fenlei(e){
          ...
          console.log(text)
           if(text == '全部'){
              const {data:res} = await this.$axios.get('/artist/list?limit=100&area='+this.singeGategoryParams.area);
            console.log(res)
            this.singeList = res.artists;
            return;
          }
      ...
      async shuaixuan(e){
          ...
          this.singeGategoryParams.initial = text;
              const {data:res} = await this.$axios.get('/artist/list?limit=100&initial='+this.singeGategoryParams.initial);
            console.log(res)
            this.singeList = res.artists;
