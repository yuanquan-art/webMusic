    ...
    <el-tab-pane label="收藏者" name="third">
        <div>
            <div v-for="(item,index) in subscribers" :key="index">
                <img :src="subscribers.avatarUrl" alt="" width="50" height="50">
...
export default {
  ...
  methods: {
    ...
    async getMusicListHidden(){
        ...
        for(let i=0;i<this.subscribers.length-1;i++){
            //  this.parseSubscribers[i].avatarUrl = this.subscribers[i].avatarUrl;
            //  this.parseSubscribers[key].nickname = this.subscribers[key].nickname;
            console.log(this.subscribers[i].avatarUrl)
            console.log(this.subscribers[i].nickname)
        }
    }
  },
};
