...
export default {
  ...
  methods: {
    ...
    async postPinlunContent(){
        if(this.commentid){
            console.log(this.content.toString().split(':')[1])
             const {data:res} = await this.$axios.get('http://localhost:3000/comment?t=2&type=2&id='+this.commentid+'&content='+this.content);
             console.log(res)
