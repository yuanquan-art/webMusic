    ...
    <el-tab-pane label="歌曲评论" name="second">
        ...
        <ul>
            <li class="punlunLi" v-for="(item,index) in hotComments" :key="index">
                ...
                <span class="zan">
                    <i :class="(item.liked?'iconfont iconweibiaoti- zanzan':'iconfont iconweibiaoti-')" @click="dianzan(item.commentId,item.liked)"  ref="zan"></i>
                    <i >{{ item.likedCount }}</i>
