...
export default {
  ...
  computed: {
    ...
    //     .toLocaleString()
    //     .replace(/:\d{1,2}$/, " ");
     return `${this.musicListXQ.createTime.getFullYear()}年${this.musicListXQ.createTime.getMonth()+1}月${this.musicListXQ.createTime.getDate()}日`
    },
  },
  methods: {
    // 获取歌单详情
    async getMusicListXQ() {
      ...
      this.musicListXQ.tracks.forEach(element => {
          element.like = false;
          // 转化时间戳为时长
          const nowDay = new Date( element.publishTime);
          element.publishTime = `${nowDay.getFullYear()}年${nowDay.getMonth()+1}月${nowDay.getDate()}日${nowDay.getHours()}时${nowDay.getMinutes()}分${nowDay.getSeconds()}秒`;
          console.log(element.publishTime)
      });
