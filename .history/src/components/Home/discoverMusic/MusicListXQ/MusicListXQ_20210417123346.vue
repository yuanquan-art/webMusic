    ...
    <el-tab-pane label="收藏者" name="third">
        <div>
            <div v-for="(item,index) in parseSubscribers" :key="index">
                ...
                <span>{{ JSON.parse(parseSubscribers).nickname }}</span>
            </div>
        </div>
    </el-tab-pane>
  </el-tabs>
...
export default {
  ...
  methods: {
    ...
    async getMusicListHidden(){
        ...
        console.log(res)
        this.parseSubscribers = res.subscribers;
        console.log(this.subscribers)
    
        for(const key in this.subscribers){
             this.parseSubscribers[key].avatarUrl = this.parseSubscribers[key].avatarUrl;
             this.parseSubscribers[key].nickname = this.parseSubscribers[key].nickname;
        }
