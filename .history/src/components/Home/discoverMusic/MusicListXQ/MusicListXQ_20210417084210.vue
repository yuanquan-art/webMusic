    ...
    <el-tab-pane label="歌曲评论" name="second">
        ...
        <ul>
            <li class="punlunLi" v-for="(item,index) in hotComments" :key="index">
                ...
                <span class="zan">
                    <i class="iconfont iconweibiaoti-" @click="dianzan(item.commentId,item.liked)"></i>
                    <i >{{ item.likedCount }}</i>
                    <i class="iconfont iconhuifu" id="huifu"></i>
                </span>       
            </li>
        </ul>
        <p class="toJingchai">
...
export default {
  ...
  methods: {
    ...
    // 点赞
    async dianzan(commentId){
      console.log(commentId)
      const {data:res} = await this.$axios.get('http://localhost:3000/comment/like?id=2'+ this.$route.query.id+'&cid='+commentId+'&t=1&type=2');
      console.log(res)
    }
  },
};
