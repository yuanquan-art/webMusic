...
export default {
  ...
  methods: {
    ...
    async getMusiclistPinlun(){
     ...
     for(let i =0;i<10;i++){
         const nowDay = res.hotComments[i].time;
         res.hotComments[i].time=`${nowDay.getFullYear()}年${nowDay.getMonth()+1}月${nowDay.getDate()}日${nowDay.getHours()}时${nowDay.getMinutes()}分${nowDay.getSeconds()}秒`
