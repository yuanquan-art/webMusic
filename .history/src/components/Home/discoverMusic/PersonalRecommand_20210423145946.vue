...
export default {
  ...
  methods: {
    ...
    // 点击播放推荐新音乐
    async dbBofangxinMusic(item){
      const {data:res} = await this.$axios.get('http://localhost:3000/song/url?id='+item.id);
      const {data:lrc} = await this.$axios.get('http://localhost:3000/lyric?id='+item.id);
      this.musicObjParams.lrc = lrc.lrc.lyric;
     this.musicObjParams.title = item.name;
      this.musicObjParams.author = item.song.album.artists[0].name;
      this.musicObjParams.pic = item.picUrl;
