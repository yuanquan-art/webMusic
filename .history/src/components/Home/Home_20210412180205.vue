...
export default {
  setup(props, context) {
    ...
    const handleSearch =async () => {
          const { data: res } = await axios.post("http://localhost:3000/cloudsearch?keywords="+params.value);
          console.log(res)
           router.push('/searchhome?state._value=' + params.value); 
         // router.push({name: '/searchhome',params: res});
    }
    return {
      //登录
      loginDialogVisible,
