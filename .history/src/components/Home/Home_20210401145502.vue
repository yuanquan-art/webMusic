        ...
        <el-row>
          ...
          <el-col :span="9" class="top-right">
            <el-row class="top-zujian">
             ...
             <el-col :span="1"><!-- 邮箱 -->
                <i class="iconfont iconyouxiang"></i></el-col>
...
>
  ...
  <el-form-item  label="电话" prop='phone'>
    ...
    <el-input class="zhucePhone" v-model="signUpForm.phone" ></el-input>
  </el-form-item>
  <el-form-item label="验证码" prop="code">
    <el-input class="yanzhenInput" v-model="signUpForm.code"></el-input>
    <el-button type="primary" @click="getSignupCode" class="fayanzheng">发送验证码</el-button>
  </el-form-item>
  <el-button class="zhuceButton" type="primary" @click="zhuceMusicId">注册</el-button>
  </el-form>
</el-dialog>
...
export default {
  setup(props,context){
     ...
     const zhuceMusicId =async () => {
       //验证手机号和验证码是否正确
        const {data:res} = await axios.get('captcha/verify?phone='+signUpForm.phone+'&captcha='+signUpForm.code);
        if(res.code !== 200){
          ElMessage.error('手机号和验证码不匹配！');
        }
        ...
        if(zhuceRes.code == 200){
           signUpDialogVisible.value =false;
           ElMessage.success('注册成功');
           loginDialogVisible.value = true;
           return;
        }
        ElMessage.error('该昵称已被占用');
     }
     // 登录表单验证规则
