      ...
      <el-header>
        <el-container>
          ...
          <el-main class="biaotiMessage">
            <p>
              <span class="gedanspan">付费精品</span>
            ...
            <p>
              <!-- <img
                ...
                class="avatarUrl"
              /> -->
              <span class="nickname">￥{{ XQCreatorNickName/100 }}</span>
            </p>
            <p>
              <span class="quanbubofang">立即购买</span>
    ...
    <el-tab-pane label="节目" name="first">
       ...
       > 
          ...
          <span class="liserntCount">
              <i></i>
              <span>{{ parseInt(item.listenerCount) }}</span>
          </span>
...
export default {
  ...
  methods: {
    ...
    async getMusicListHidden(){
        const {data:res} = await this.$axios.get('http://localhost:3000/dj/subscriber?id='+this.$route.query.id);
