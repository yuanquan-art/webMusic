  ...
  methods: {
    ...
    async getVideoXQ() {
      const { data: res } = await this.$axios.get(
        "http://localhost:3000/video/detail?id=" + this.$route.query.id
      );
    ...
    //   this.mvXQdataArtistsName2 = this.mvXQdata.artists[1].name;
      //请求mv地址的url
      const {data:resUrl} = await this.$axios.get('http://localhost:3000/video/url?id='+res.data.id);
    ...
    async getMusiclistPinlun() {
      const { data: res } = await this.$axios.get(
        "http://localhost:3000/video/detail/info?vid=" + this.$route.query.id
      );
      for (let i = 0; i < 8; i++) {
